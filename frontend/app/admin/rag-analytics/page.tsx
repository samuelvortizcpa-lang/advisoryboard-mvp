  ...
  const refreshData = useCallback(async () => {
    try {
      const [s, e] = await Promise.all([
        apiFetch<EvalSummary>("/rag-analytics/summary?days=30"),
        apiFetch<EvalListItem[]>("/rag-analytics/evaluations?limit=20"),
      ]);
      setSummary(s);
      setEvals(e);
    } catch {
    ...
    async (evaluationId: string) => {
      ...
      try {
        const detail = await apiFetch<EvalDetail>(
          `/rag-analytics/evaluations/${evaluationId}`,
        );
